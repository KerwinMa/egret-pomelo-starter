...
export default class SessionMediator extends puremvc.Mediator implements puremvc.IMediator {

    public static instance: SessionMediator = null;
    public static getinstance (app:any, mediatorName?: string) {
        if (!this.instance) this.instance = new SessionMediator(app, mediatorName);
    ...
    @handler('session.remote.handler', __filename)
    public createSession (session: any, socket: any, cb: Function) {
        console.log(session, socket);
        socket.body = 'hahahahahah';
